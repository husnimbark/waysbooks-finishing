const { user, transaction, book, profile } = require("../../models");
...
exports.getTransactions = async (req, res) => {
  try {
    ...
    let data = await transaction.findAll({
      ...
      attributes: {
        exclude: ["updatedAt", "idBuyer", "idSeller", "idBook"],
      },
      include: [
        {
          model: book,
...
exports.addTransaction = async (req, res) => {
  try {
    ...
    data = {
      id: parseInt(data.idBook + Math.random().toString().slice(3, 8)),
      ...data,
      idBuyer: req.user.id,
      status: "pending",
    };
...
const updateBook = async (orderId) => {
  ...
  const bookData = await book.findOne({
    where: {
      id: transactionData.idBook,
    },
  });
  const qty = bookData.qty - 1;
  await book.update({ qty }, { where: { id: bookData.id } });
};
